...
def helper(dis):
    ...
    desc = " ".join([w for w in desc_series]) if not desc_series.empty else "No description available"
...
st.markdown("Enter your symptoms separated by commas (e.g., headache, fever, cough):")
...
if st.button("Predict Disease"):
    if user_input:
        ...
        filtered_symptoms = [sym for sym in user_symptoms if sym in symptoms_dict]
        ...
        if filtered_symptoms:
            ...
            if wrkout:
                with st.expander("🏋️‍♀️ Workout Suggestions"):
                    for item in wrkout:
                        st.markdown(f"🕺 **{item}**")

        else:
            st.warning("⚠️ No valid symptoms found. Please check for spelling.")
    else:
